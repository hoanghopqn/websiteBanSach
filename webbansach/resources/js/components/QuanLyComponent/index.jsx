...
import React, { useEffect, useState } from "react";
import { Button, Tab, TabContainer, Tabs } from "react-bootstrap";
import "./style.scss";
import NhanVien from "./NhanVien";
import KhachHang from "./KhachHang";
import DSTaiKhoan from "./DSTaiKhoan";
...
function QuanLyComponent(props) {
    const [tabKey, setTabKey] = useState("info"); // Initial tab key

    const initState = useSelector((state) => state.sanpham);
    let navigate = useNavigate();
    const { listNhanVien, listKhachHang, listSanPham, listDSTaiKhoan, listGiamGia, listPhieuNhap, listDDH, listTacGia, listNuoc } = initState;
    const nameQL = [
        { key: "info", name: 'Thông Tin Cá Nhân', icon: <FaBookReader /> },
        { key: "product", name: 'Quản Lý Sản Phẩm', icon: <FaBookReader /> },
        { key: "employee", name: 'Quản Lý Nhân Viên', icon: <FaBookReader /> },
        { key: "customer", name: 'Quản Lý Khách Hàng', icon: <FaUserCog /> },
        { key: "account", name: 'Danh Sách Tài Khoản', icon: <FaUserCog /> },
        { key: "discount", name: 'Quản Lý Giảm Giá', icon: <FaGift /> },
        { key: "import", name: 'Quản Lý Phiếu Nhập', icon: <FaUserCog /> },
        { key: "order", name: 'Quản Lý Đơn Đặt Hàng', icon: <FaUserCog /> },
        { key: "author", name: 'Quản Lý Tác Giả', icon: <FaUserCog /> },
        { key: "country", name: 'Quản Lý Nước', icon: <FaUserCog /> },
    ];

    const nhanvien = JSON.parse(localStorage.getItem("nhanvien"));
    const urlImg = `/images/${nhanvien.user.hinhanh != null ? nhanvien.user.hinhanh : "user"}.jpg`;
    ...
    return (
        <div className="feature d-flex h-100">
            <Tabs activeKey={tabKey} onSelect={(k) => setTabKey(k)} className="flex-column">
                {nameQL.map(tab => (
                    <Tab eventKey={tab.key} title={tab.name} key={tab.key}>
                        ...
                        {tab.key === "info" && <DetailNhanVien />}
                        {tab.key === "product" && (
                            <div className="tab-content-container m-3">
                                <SanPham sanpham={listSanPham.listSanPhamdata} lengthma={listSanPham.meta.total} />
                                <Pagination getFilterLimit={getSPFilterLimit} getFilterPage={getSPFilterPage} meta={listSanPham.meta} />
                            </div>
                        )}
                        {tab.key === "employee" && nhanvien.quyen === 3 && (
                            <div className="tab-content-container m-3">
                                <NhanVien nhanvien={listNhanVien.listNhanViendata} dstaiKhoan={listDSTaiKhoan} />
                                <Pagination getFilterLimit={getNVFilterLimit} getFilterPage={getNVFilterPage} meta={listNhanVien.meta} />
                            </div>
                        )}
                        {tab.key === "customer" && nhanvien.quyen === 3 && (
                            <div className="tab-content-container m-3">
                                <KhachHang khachhang={listKhachHang.listKhachHangdata} dstaiKhoan={listDSTaiKhoan} />
                                <Pagination getFilterLimit={getKHFilterLimit} getFilterPage={getKHFilterPage} meta={listKhachHang.meta} />
                            </div>
                        )}
                        {tab.key === "account" && nhanvien.quyen === 3 && (
                            <div className="tab-content-container m-3">
                                <DSTaiKhoan dstaiKhoan={listDSTaiKhoan.listDSTaiKhoandata} />
                                <Pagination getFilterLimit={getDSTaiKhoanFilterLimit} getFilterPage={getDSTaiKhoanFilterPage} meta={listDSTaiKhoan.meta} />
                            </div>
                        )}
                        {tab.key === "discount" && <GiamGia giamgia={listGiamGia} nhanvienid={nhanvien.user.id} hotenid={nhanvien.user.hoten} />}
                        {tab.key === "import" && (
                            <div className="tab-content-container m-3">
                                <PhieuNhap phieunhap={listPhieuNhap.listPhieuNhapdata} />
                                <Pagination getFilterLimit={getPNFilterLimit} getFilterPage={getPNFilterPage} meta={listPhieuNhap.meta} />
                        ...
                        {tab.key === "order" && (
                            <DDHComponent dsdondathang={listDDH.listDDHdata} filterDDHs={listDDH.filter} nhanvienid={nhanvien.user.id} />
                        )}
                        {tab.key === "author" && (
                            <div className="tab-content-container m-3">
                                <TacGia tacgia={listTacGia.listTacGiadata} />
